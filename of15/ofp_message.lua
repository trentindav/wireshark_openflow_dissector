...
function ofp_multipart_request(tvb, pinfo, tree)
    ...
    local flags_tree = subtree:add(fields.multipart_flags, tvb(2,2))
    flags_tree:add(fields.multipart_flags_request_more, tvb(2,2))
...
function ofp_multipart_reply(tvb, pinfo, tree)
    ...
    subtree:add(fields.multipart_type, tvb(0,2))
    local flags_tree = subtree:add(fields.multipart_flags, tvb(2,2))
    flags_tree:add(fields.multipart_flags_reply_more, tvb(2,2))
    -- subtree:add(fields.pad4, tvb(4,4))
    if (tvb():len() > 8) then
